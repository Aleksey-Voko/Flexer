...
# Числ-ные с дефисом. Изм. обе части. Идентификаторы.txt
def get_numerals_hyphenated_identifiers(word_forms_bases, _) -> list:
    """
    1. Создать документ Числ-ные с дефисом. Изм. обе части.txt .
    2. Найти в БС группы с ЗС из документа
        Числ-ные с дефисом. Изм. обе части.txt .
    3. Создать документ Числ-ные с дефисом. Изм. обе части. Идентификаторы.txt
    ...
    for group in word_forms_bases:
        if (str(group.title_word_form) in numerals_hyphenated_ch_both_parts
                and group.title_word_form.idf.startswith('.Ч')):
            forms = [group.title_word_form] + group.word_forms
            idfs = [x.idf for x in forms]
